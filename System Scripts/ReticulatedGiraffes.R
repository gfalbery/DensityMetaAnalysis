...
FocalSystem <- "ReticulatedGiraffes"
...
Giraffes <- FileList %>% last

Locations <- Files[2] %>% read_xls

Locations %<>% 
  rename_all(~str_replace_all(.x, " ", "_")) %>% 
  mutate(X = Lon*100, Y = Lat*100) %>% 
  mutate(X = X - min(X), Y = Y - min(Y)) # Remember to put it in kilometres!!!!!!!!

Locations$Year <- Locations$Date %>% 
  str_split("-") %>% map_chr(1) %>% as.numeric

# Deriving stuff ####

Censuses <- Locations %>% 
  mutate(GroupDate = Sighting_Number)

# Making data frame

ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = Giraffe_IDs, GroupDate, X, Y, Year) %>% 
  na.omit
