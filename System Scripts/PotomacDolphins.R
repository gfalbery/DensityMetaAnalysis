...
Coastals <- 
  Root %>% dir_ls(regex = "csv") %>% read.csv %>% 
  separate(Date, sep = " ", into = c("Date", "Group")) %>% 
  mutate_at("Date", ~lubridate::dmy(.x)) %>% 
  mutate(Coastal = "Y") %>% 
  mutate(GroupDate = paste0(Date, "_", Group))

# Melissa Guidance ####

names(SheetList) <- c("2016_2017", "2015", "2018", "2019", "Archive")

SheetList <- SheetList[1:3]

SheetList %<>% map(as.data.frame)

SheetList[[1]] %<>% 
  filter(!str_detect(Number_ID, " or ")) %>% 
  mutate(ID = Number_ID, GroupDate = paste0(Sighting_Date, "_", Observation_ID), 
         Lat = Sighting_Latitude, Lon = Sighting_Longitude) %>% 
  dplyr::select(ID, GroupDate, Lat, Lon, Date = Sighting_Date) %>% 
  left_join(Coastals %>% dplyr::select(Coastal, GroupDate), 
            by = c("GroupDate"))

SheetList[[2]] %<>% 
  mutate(ID = `Contributor's dolphin identification code`, 
         # GroupDate = paste0(`Sighting date`, "_", Observation_ID), 
         Lat = Latitude, Lon = Longitude, Date = `Sighting date`) %>% 
  arrange(Date, Lon, Lat, ID) %>% 
  mutate(GroupDate = as.character(as.numeric(as.factor(paste(Date, Lon, Lat, sep = "_"))))) %>%
  dplyr::select(ID, GroupDate, Lat, Lon, Date) %>% 
  left_join(Coastals %>% dplyr::select(Date, LonLat, Coastal) %>% 
              separate(LonLat, sep = "/ ", into = c("Lat", "Lon")) %>% 
              mutate_at(c("Lat", "Lon"), as.numeric), 
            by = c("Date", "Lat", "Lon"))  

Dolphins <- 
  SheetList[1:2] %>% bind_rows() %>% 
  mutate(Year = str_split(Date, "-") %>% map_chr(1)) %>% arrange(Date) %>% 
  filter(!Year == "2014") %>% 
  mutate_at("Coastal", ~replace_na(.x, "N"))
...
ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = ID, GroupDate, X, Y, 
                Year) %>% 
  na.omit
