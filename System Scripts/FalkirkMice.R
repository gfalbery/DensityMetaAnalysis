...
library(ggplot2); library(INLA); library(tidyverse); library(magrittr); library(ggregplot); library(gsheet)
library(igraph); library(adehabitatHR); library(rgeos); library(fs)

# rm(list = ls())

FocalSystem <- "FalkirkMice"
...
Spouce <- read.csv(paste0(Root, "/SpouceDF.csv"), header=T)

colnames(Spouce) <- CamelConvert(colnames(Spouce))

Phen <- c("Grid", "Pittag", "Easting", "Northing")

Covar <- c("Month", "Year", #"Age", #"Repro_bin", "Condition", 
           "Supp_corrected", "Treated")

Spouce %>% dplyr::select(Hpoly_epg:Albumin_conc) %>% names %>%
  setdiff("Hymenolepid_epg") -> 
  
  Resps

Spouce %>% mutate_at(Resps[2:6], AsBinary) -> Spouce

Spouce %>% mutate(X = Easting, Y = Northing) -> Spouce

Spouce %<>% mutate_at(c("X", "Y"), ~.x - min(.x))

Spouce %<>% mutate_at(c("X", "Y"), ~.x/1000)

Spouce %<>% rename(Date = Jandate)

Spouce %<>% mutate_at("Pittag", ~paste0("X", .x))
...
ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = Pittag, Date, GroupDate, X, Y, Year) %>% 
  na.omit
...
for(i in i:length(GroupList)){
  ...
  Dates %>% map(function(j){
    ...
    ifelse(SubMat < (((10^2 + 10^2)^(1/2))/1000), 1, 0) %>% 
      ...
      dplyr::select(-value) %>% 
      mutate(Date = j) %>% mutate_at(c("Var1", "Var2"), as.character)
...
NodeDF %<>% mutate_at("Year", as.character) %>% 
  left_join(SocialNodeTraits, by = c("Id", "Year"))
...
NodeDF %<>% filter(Density.Annual > 0.0025)
