...
Dates <- read.csv(paste0(Root, "/Data/Dates.csv"))
...
RawSetts <- read.csv(paste0(Root, "/Data/RawSetts.csv"))

RawSetts %>% mutate(Sett.ID = Sett %>% str_split(",") %>% map(2) %>% substr(7,20) %>% str_remove(".$"),
                    X = Sett %>% str_split(",") %>% map(4) %>% str_split(":") %>% map(2) %>% substr(2, nchar(.)) %>% as.numeric,
                    Y = Sett %>% str_split(",") %>% map(5) %>% unlist() %>% as.numeric()) %>%
  dplyr::select(-Sett) -> RawSetts2

ReportedSetts <- read.csv(paste0(Root, "/Data/ReportedSetts.csv"))

ReportedSetts %>% 
  mutate_at(c("XCoordinate", "YCoordinate"), ~ifelse(.x>5000, .x/10, .x)) %>% 
  filter(XCoordinate>0)  -> ReportedSetts

ReportedSetts %>% mutate_at("Code", ~.x %>% str_trim %>% toupper) -> ReportedSetts

# BadgerCaptures #### 

BadgerCaptures <- read.csv(paste0(Root, "/Data/dataForGreg.csv"))

BadgerCaptures %>% 
  mutate(Trapping.Title = Trapping.Title %>% str_remove("Additional ")) %>%
  separate(Date, "-", into = c("fYear", "Month", "Day")) %>%
  mutate(Date = BadgerCaptures$Date) %>% 
  mutate(Year = as.numeric(fYear)) -> BadgerCaptures

BadgerCaptures %>% 
  filter(as.numeric(Month) %in% 5:11) ->
  BadgerCaptures
...
SettLocations <- read.csv(paste0(Root, "/Data/ReportedSetts.csv"))

SettLocations %>% 
  mutate_at(c("Name", "Code"), ~.x %>% str_trim %>% toupper) %>% 
  rename(Sett = Name, X = XCoordinate, Y = YCoordinate) %>%
  filter(!Sett %in% toupper(c("Beacon Hill", "Mac Braken", "Botley Lodge"))) -> 
...
SettLocations %>% 
  dplyr::select(-Code) %>% 
  filter(X>0) %>% 
  mutate_at(c("X", "Y"), ~ ifelse(.x > 5000, .x/10, .x)) %>% 
  mutate_at(c("X", "Y"), ~ .x/100) %>% 
  mutate_at(c("X", "Y"), ~ .x - min(.x, na.rm = T)) -> 
  
  SettLocations

BadgerCaptures %<>% left_join(SettLocations)
