...
(Sharks <- Files[[2]] %>% rename(Shark = 1)) %>% head

Stations <- Files %>% last %>% rename(Site = 1)

Sharks %<>% left_join(Stations %>% dplyr::select(-Location), ., by = c("Site"))

(Locations <-
    Sharks %>% 
    mutate(GroupEvent = Group) %>% 
    arrange(GroupEvent))
...
ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = Shark, GroupDate, X, Y, 
                Year) %>% 
  na.omit
