...
Files <- 
  Root %>% 
  dir_ls(regex = ".csv") %>% map(read.csv)

Groups <- Files[[1]]

Groups %<>% 
  rename(ID = Anon.Dolphin.ID, 
         GroupDate = Observation.ID, 
         Lat = Latitude, Lon = Longitude) %>% 
  dplyr::select(ID, GroupDate, Lat, Lon, Date = Observation.Date) %>% 
  filter(!ID == "UNKNOWN") %>% filter(!is.na(Lon), !is.na(Lat))

Dolphins <- 
  Groups %>% 
  mutate(Year = str_split(Date, "-") %>% map_chr(1)) %>% arrange(Date)

Dolphins %<>% filter(between(Lat, -25.9, -25.65), Lon < 113.83)
