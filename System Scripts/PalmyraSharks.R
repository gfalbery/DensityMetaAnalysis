...
(Sharks <- Files %>% last) %>% head

Stations <- Files %>% first %>% dplyr::select(-Location)

Sharks %<>% full_join(Stations, ., by = c("Code" = "Location"))

(Locations <-
    Sharks %>% 
    mutate(GroupEvent = 1:n()) %>% 
    gather("Shark", "Value", X61343:X13996) %>% 
    filter(Value == 1) %>% arrange(GroupEvent, Shark))
...
Censuses <- 
  Locations %>% 
  mutate(GroupDate = GroupEvent)

# Making data frame

ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = Shark, GroupDate, X, Y, Year) %>% 
  na.omit
