...
Dolphins %<>% 
  filter(START_LAT > 57.35, START_LONG < -3.5)# %>% 
#nrow
# filter(START_LAT > 56.25, START_LAT < 56.72, ORGANISATION == "SMRU") %>%
# ggplot(aes(START_LONG, START_LAT)) + 
# geom_pointdensity() + 
# coord_sf()
...
Dolphins %<>% as.data.frame

Dolphins %<>% mutate(GroupDate = as.numeric(factor(paste(Date, X, Y))))
...
ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = Idno, GroupDate, X, Y, 
                Year) %>% 
  na.omit
