...
Spouce <- read.csv(paste0(Root, "/Silwood_capture_metadata14-15_cleaned_AR.csv"))

Spouce %<>% 
  filter(str_count(Grid_square) < 4) %>% 
  filter(str_detect(Grid_square, paste(LETTERS, collapse = "|")))

NEWLETTERS <- c("O", "N", "M", "L", "K", "J", "I", "H", "G", "F", "E", "D", "C", "B", "A", "Z", "Y", "X", "W", "V")

Spouce %<>% 
  mutate(X = Grid_square %>% substr(1, 1) %>% factor(levels = NEWLETTERS) %>% as.numeric) %>% 
  mutate(Y = Grid_square %>% substr(2, 3) %>% as.numeric)

Spouce %<>% filter(X < 20)

Spouce %>% rename_all(CamelConvert) %>% 
  mutate(Date_clean = Trapdate %>% str_split(" ") %>% map_chr(last)) %>%  
  mutate(Date = Date_clean) %>% 
  separate(Date_clean, sep = "/", into = c("Day", "Month", "Year")) ->
  Spouce
