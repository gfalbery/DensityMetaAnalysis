...
(Stations <- Files %>% last) %>% head

Sharks <- Files %>% first# %>% dplyr::select(-Location)

Observations <- Files[[2]]# %>% first# %>% dplyr::select(-Location)

LongObservations <- Observations %>% 
  mutate(GroupObs = 1:n()) %>% 
  pivot_longer(X27601:ncol(Observations)) %>% filter(value > 0)

LongObservations %<>% full_join(Stations, by = c("Location" = "Station_name"))

# LongObservations %<>% 
#   mutate(X = X*100, Y = Y*100) # Remember to put it in kilometres!!!!!!!!

LongObservations %<>% filter(!is.na(Start))

LongObservations$Year <- 
  LongObservations$Start %>% 
  str_split("/") %>% 
  map_chr(3) %>% 
  str_split(" ") %>% 
  map_chr(1) %>% 
...
LongObservations %<>% filter(Year > 2013)

# Deriving stuff ####

Censuses <- 
  LongObservations %>% 
  mutate(GroupDate = GroupObs)

# Making data frame

ObservationDF <- Censuses %>% 
  # dplyr::select(-Year) %>% 
  dplyr::select(Id = name, GroupDate, X, Y, Year) %>% 
  na.omit
